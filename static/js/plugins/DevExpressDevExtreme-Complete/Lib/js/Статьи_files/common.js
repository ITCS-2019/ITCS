...
(function($) {
	$.fn.autocomplete = function(option) {
		...
		/*	$(document).on('mouseover', '.autocomplete', function(){
                document.getElementById('input-filter').request();
                console.log(this.id);
            });
*/
			// Keydown
			...
			this.response = function(json) {
				...
				if (json.length) {
					for (i = 0; i < json.length; i++) {
						...
						if (!json[i]['category']) {
							// ungrouped items
							html += '<li data-value="' + json[i]['value'] + '"><a href="#">' + json[i]['label'] + ' ( ' + json[i]['value'] + ' ) ' +'</a></li>';
						} else {
							// grouped items
